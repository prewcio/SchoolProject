<template>
  <div class="container test" v-if="this.success === 1">
    <div class="exeTest">
      <div
        v-if="this.currentUrl === '/' + this.userName + '/exercise'"
        class="exercises"
      >
        <h1>Exercises</h1>
        <form
          id="checkWord"
          @submit.prevent="exerciseCheck()"
          v-if="correct !== 2"
        >
          <h2>"{{ currentWord }}"</h2>
          <input
          ...
          />
          <h3 style="color: red" v-if="correct === -1">
            Nope! Correct: {{ translatedWord }}
          </h3>
          ...
          />
          <button
            @click="excludeExercise(currentWord.toLowerCase())"
            v-if="correct !== 0"
          >
            Przejdź dalej
          </button>
        </form>
        <h3 style="color: green" v-if="correct === 2">
          You have answered all questions correctly!
        </h3>
        <button v-if="correct === 2" @click="backRedirect()">Go back!</button>
      </div>
      <div
        v-else-if="this.currentUrl === '/' + this.userName + '/test'"
        class="tests"
      >
        <h1>Tests</h1>
...
export default {
  ...
  mounted() {
    this.checkIfLogged();
    axios.get("/api/en/getWords").then((response) => {
  ...
  methods: {
    exercises: function () {
      ...
      if (!this.excludeWordsExer.includes(this.wordsPool[rand])) {
        this.currentWord =
          this.wordsPool[rand].charAt(0).toUpperCase() +
          this.wordsPool[rand].slice(1);
        this.translatedWord =
          this.transWordsPool[rand].charAt(0).toUpperCase() +
          this.transWordsPool[rand].slice(1);
      } else {
        this.exercises();
      }
    },
